const steps = [
  ...
  },
  {
    ...
    actor: "Credence",
    message:
      "Merchant 4827: coffee shop, Hanoi, 18 months active, $1,200/mo revenue, 94% activity rate. Missing: loan amount and number of dependents — these have the highest impact on the score. Could you provide them?",
    note: "Data Completeness Check → 2 missing fields identified",
  },
  {
    index: "03",
    actor: "Loan Officer",
  ...
  },
  {
    ...
    note: "Credit Scoring → Factor Analysis → Improvement Paths",
  },
];
...
const scoreBands = [
  ...
  { range: "300–579", band: "Poor", decision: "Decline + improvement guidance", color: "bg-red-500/20 text-red-400 border-red-500/20" },
];
            ...
            {steps.map((step) => (
              ...
              >
                <div className="shrink-0 mt-0.5">
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-mono font-medium border ${
                      step.actor === "Loan Officer"
                        ? "border-white/10 text-white/30 bg-white/[0.03]"
                        : "border-white/20 text-white/60 bg-white/[0.06]"
                    }`}
                  >
                    {step.actor === "Loan Officer" ? "LO" : "AI"}
                  </div>
                </div>
                <div
